...
void driveLine() {  //This function will lead the Zumo once the line is found.
  ...
  while (driveMode) {
    switch (sensorOnLine()) {
      case 0:                 //Normal mode 1. This runs of no sensor is on the line
        softTurn(direction);  //Turn into the line.
        Serial.print("case 0");
        break;
      case 1:                  //Normal mode 2. This runs if only sensor 0 is on the line.
        softTurn(!direction);  //Turn away from the line.
        Serial.print("case 1");
        startTime = millis();
        break;
      case 2:  //Error mode. Will turn away from the line until sensor 0 looses the line on the OUTSIDE of itself. This runs if only sensor 1 is on the line.
        while (sensorOnLine() != 1) {
          softTurn(!direction);  //Turn away from the line.
          Serial.println("case 2 part1");
          delay(50);
        }
        while (sensorOnLine() == 1) {
          softTurn(!direction);  //Turn away from the line.
          Serial.println("case 2 part2");
          delay(50);
        }
        break;
...
void loop() {
  ...
  if (buttonC.getSingleDebouncedRelease()) {  //Button C press print test array rn. It will also often cancel a running mode.
    buttonC.waitForRelease();
    printArrayToSerial(lineSensorMin);
